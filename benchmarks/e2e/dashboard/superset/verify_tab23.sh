...
# --- 8. VIZ-TYPE ALLOWLIST GUARD (regression guard, 2026-07-13) ----------------
#   Superset viz keys are per-BUILD: `histogram` and `dist_bar` are UNREGISTERED in
#   the DWH Superset build and threw a viz ERROR (not an honest empty state) — the
#   exact bug this guard prevents. Assert every chart viz_type in the Tab-2/Tab-3
#   deploy payloads is in the documented allowlist of keys REGISTERED in this build
#   (see DASHBOARD.md "VIZ-TYPE ALLOWLIST" standing rule). A future chart that
#   reaches for an unregistered viz type FAILS the harness here.
#   The guard is proven live by planting a `histogram` chart into a throwaway copy of
#   the payload and asserting the checker rejects it (assert-the-assertion).
viz_check() { # <deploy-json-path> ; prints "OK" or "BAD:<vt>[,<vt>...]"
  python3 - "$1" <<'PY'
import json, sys
ALLOW = {
    # REGISTERED + render-empty-graceful in this Superset build (known-good):
    "big_number_total", "echarts_timeseries_line", "echarts_timeseries", "table",
    "markdown",
    # VERIFY-BEFORE-USE (registered but have thrown in some states — allowed but
    # flagged in DASHBOARD.md; keep here so existing charts pass, review before adding):
    "mixed_timeseries", "echarts_timeseries_bar", "echarts_timeseries_scatter",
}
d = json.load(open(sys.argv[1]))
bad = sorted({c.get("viz_type") for c in d.get("charts", [])
              if c.get("viz_type") not in ALLOW})
print("OK" if not bad else "BAD:" + ",".join(bad))
PY
}
for f in tab2_deploy.json tab3_deploy.json tab4_deploy.json tab5_deploy.json; do
  res="$(viz_check "$SCRIPT_DIR/deploy/$f")"
  check "viz-allowlist: $f every viz_type registered" "OK" "$res"
done
# assert-the-assertion: a planted `histogram` chart MUST be rejected.
PLANT="$(mktemp)"; python3 - "$SCRIPT_DIR/deploy/tab2_deploy.json" "$PLANT" <<'PY'
import json, sys
d = json.load(open(sys.argv[1]))
d["charts"].append({"slice_name": "kc_planted_bad", "viz_type": "histogram",
                    "dataset": "v_kc_inserts_drill", "params": {}})
json.dump(d, open(sys.argv[2], "w"))
PY
planted="$(viz_check "$PLANT")"; rm -f "$PLANT"
check "viz-allowlist: planted histogram is REJECTED (guard works)" "BAD:histogram" "$planted"
