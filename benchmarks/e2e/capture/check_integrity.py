...
Ported verbatim from spark-clickhouse-connector benchmarks/scripts/check_integrity.py.
...
run's fate: per contract §3 an integrity mismatch FAILS the run outright (unlike
the flagged-not-failed guards), but the metrics must survive for the dashboard,
so the failure lives here rather than in a rollback-triggering capture failure.
