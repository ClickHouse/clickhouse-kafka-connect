...
Ported verbatim from spark-clickhouse-connector benchmarks/scripts/wait_for_settle.py.

After the drain reaches lag 0, ClickHouse keeps merging the small parts in the
background for minutes-to-hours. This waits until the active part count is stable
AND no merges are in flight (for STABLE_SAMPLES consecutive polls) or
...
orchestrator can capture it as `SETTLE_END=$(wait_for_settle.py)`. All progress
...
settle_seconds value is right-censored). The orchestrator reads this back into
the settle_timed_out flag (contract §1.3 token settle_timeout) so censored settle
values can be excluded from trends; a stdout-only contract can't carry it.
