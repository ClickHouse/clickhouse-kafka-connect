...
#
"""Run one parameterized capture SQL file against the perf.* metrics service.

Ported from spark-clickhouse-connector benchmarks/scripts/run_metrics_sql.py.
Same {name:Type} parameter-substitution mechanism (clickhouse_connect binds the
params server-side; nothing is string-interpolated into SQL). Kafka adaptations:
  * The drain window {run_start}/{run_end} is bounded by the orchestrator/poller
    (connector-start -> lag-0 timestamp), not a Spark event-log window — but the
    SQL just consumes the two timestamps, so the mechanism is unchanged.
  * A new {query_user} parameter carries the connector's CH user for the
    query_log filter (see 11_insert_from_query_log.sql header). It is '' by
    default, which makes the user predicate a no-op (table scope alone).
...
Optional env: RUN_END (defaults to RUN_START — unset for pre-drain capture),
              SETTLE_END (defaults to RUN_END), SETTLE_SECONDS (default 0),
              SETTLE_TIMED_OUT (default 0),
              QUERY_LOG_USER (connector CH user to scope query_log inserts to;
                default '' => table-scope filter only),
              INPUT_PARQUET_GLOB (source glob, s3a:// or s3://; exposed to SQL as
                {source_glob} in s3:// form),
...
def resolve_expected(client, sql: str, source_glob: str):
    ...
    the workflow env (SOURCE_ROWS_EXPECTED / SOURCE_UNIQUE_EXPECTED). For the
    Kafka backlog-drain SOURCE_ROWS_EXPECTED is the producer's authoritative
    committed-offset count (plan §5 step 2); SOURCE_UNIQUE_EXPECTED is the staged
    uniqExact(WatchID) constant. Per-run re-derivation would uniqExact-scan the
    full ~100M-row source with GB-scale hash state on a memory-capped service,
    every run. Re-derivation via s3() is the fallback ONLY for non-default
    ...
    # Kafka default path: no glob at all, backlog was produced from the topic.
    # The producer's committed-offset count is the source of truth, so if the
    # constants are set we use them regardless of glob presence.
    if const_rows and const_uniq and (is_default or not raw_glob):
        print(f"integrity: using pinned source constants (rows={const_rows}, "
              f"unique={const_uniq})", file=sys.stderr)
        return float(const_rows), float(const_uniq)

    if not raw_glob:
        print("WARNING: no INPUT_PARQUET_GLOB and SOURCE_ROWS_EXPECTED / "
              "SOURCE_UNIQUE_EXPECTED are not both set — integrity source "
              "ground truth is 0/0 and the check will FAIL. Pin the producer's "
              "committed-offset count as SOURCE_ROWS_EXPECTED and the staged "
              "uniqExact(WatchID) as SOURCE_UNIQUE_EXPECTED.", file=sys.stderr)
        return 0.0, 0.0
...
def main() -> None:
    ...
    # ClickHouse's s3() wants the s3:// scheme; a Spark-style glob is s3a://.
    ...
    parameters = {
        ...
        # RUN_END is unset for pre-drain capture (e.g. pre-run covariates run
        # before the connector starts). It defaults to RUN_START then; those SQL
        # files do not reference {run_end} anyway.
