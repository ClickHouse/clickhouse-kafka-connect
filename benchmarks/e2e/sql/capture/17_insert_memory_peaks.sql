...
--
-- Ported verbatim from spark-clickhouse-connector benchmarks/sql/perf/17_insert_memory_peaks.sql
-- (server-wide metric_log / asynchronous_metric_log gauges — no query_log scope,
-- so no Kafka query_user filter is needed).
--
-- Peak server memory over the run window, so memory pressure shows up per run
-- (the per-query/per-merge peak metrics alone don't reveal the server total that
-- MEMORY_LIMIT_EXCEEDED / code 241 inserts hit). Total RSS and query memory come
-- from system.asynchronous_metric_log; the merge component is only in
