...
--
-- Ported from spark-clickhouse-connector benchmarks/sql/perf/13_insert_from_part_log.sql.
-- parts_per_insert and merge_amplification are the PINNED spellings
-- (docs/benchmark-v2-contract.md §2.1); the Spark source already renamed them
-- from ch_parts_per_insert / ch_merge_amplification per §7.
--
-- Parts + merge metrics. The window split is deliberate:
--   [run_start, run_end]    insert-time activity (parts created, ratios).
--   [run_start, settle_end] merge work, most of which happens *after* the drain
--                           ends as the merge tree consolidates.
-- {settle_end} comes from wait_for_settle.py; if unset it defaults to {run_end},
-- which under-counts merge cost. For Kafka {run_end} is the lag-0 timestamp.
...
SELECT {run_id:String}, metric_name, unit, value FROM (
  ...
  WHERE event_time BETWEEN parseDateTimeBestEffort({run_start:String}) AND parseDateTimeBestEffort({settle_end:String})
    ...
    AND database = {ch_database:String} AND table = {ch_table:String}
  UNION ALL
  -- Merge amplification: total bytes merges had to read / total bytes inserted.
  -- Healthy ~ 2x; with tiny batches can be 10x+. Denominator unions
  -- Insert + AsyncInsertFlush on written_bytes > 0 (async bytes on the flush,
  -- sync bytes on the insert), scoped by table + Kafka query_user.
  SELECT 'merge_amplification', 'ratio',
         ...
         greatest(
           ...
           1.0)
  UNION ALL
  -- Wall-clock between end-of-drain and merges-quiesced: the post-ingest tail.
  ...
  UNION ALL
  -- Sanity-check from metric_log: total merge-thread time. ProfileEvent_* in
