...
--
-- Ported from spark-clickhouse-connector benchmarks/sql/perf/15_capture_raw_inserts.sql.
--
-- Snapshots every insert into perf.ch_inserts for distribution analysis
-- (batch-size histograms, duration percentiles, and the drain-curve virtual
-- dataset v_kc_drain_curve — plan §8), so it survives the target's
-- system.query_log retention. Same Kafka query_user filter as file 11.
...
WHERE event_time BETWEEN parseDateTimeBestEffort({run_start:String}) AND parseDateTimeBestEffort({run_end:String})
  ...
  -- UNION of Insert + AsyncInsertFlush filtered to non-zero rows: the real
  -- per-batch rows live on AsyncInsertFlush in async mode, on Insert in sync.
