...
# The distinct-count key for the chaos oracle (uniqExact(WatchID) on the target).
# The streaming producer's ledger keys on this to derive SOURCE_UNIQUE_EXPECTED.
WATCHID_FIELD = "WatchID"
...
# --- continuous streaming (chaos test T4 / IC-7) -----------------------------
#
# The chaos monkey run needs a LONG-LIVED producer that streams the staged
# prefix at a bounded rate until the harness fences it (SIGTERM), then reports
# the exact distinct-WatchID count it acked. The three pieces below are kept
# PURE / injectable so the pacing, ledger, and no-wrap logic are unit-tested
# with a synthetic clock and no broker. One-shot mode never touches any of this.


class TokenBucket:
    """Token-bucket rate limiter over an injectable monotonic clock.

    Paces the stream to --rate-limit rows/s: refills continuously at ``rate``
    tokens/s, capped at ``capacity`` (default = rate, a 1-second burst). Does no
    I/O and never sleeps — when dry, the caller sleeps ``time_until(n)`` — so the
    exact same code runs bit-for-bit under a synthetic clock in tests.
    """

    # Absorb floating-point drift so the bucket cannot DEADLOCK on noise: after
    # sleeping exactly time_until(1), (t+dt)-t != dt in float, which can leave
    # tokens at 0.999999… just shy of 1 forever. A token is "available" within
    # this slack (worth <1e-9/rate seconds — negligible to the pace).
    _EPS = 1e-9

    def __init__(self, rate, capacity=None, clock=time.monotonic, initial=None):
        if rate <= 0:
...
class WatchIDLedger:
    """Exact distinct-WatchID accounting over ACKED rows (IC-7 ``unique_sent``).

    A WatchID is recorded ONLY from its delivery callback — i.e. after the
    broker acked it — so ``unique_count`` is the exact number of distinct
    WatchIDs that reached the topic, which the chaos oracle takes as
    SOURCE_UNIQUE_EXPECTED (accepted only after the rows_sent == Σ offsets
    cross-check passes; idempotence makes the acked set == the broker set).
    Holds one set of Int64 WatchIDs; on the ~2M-row chaos dataset that is
    ~100 MiB — unrelated to (and does not relax) the pyarrow/rdkafka memory
    bounds, which are unchanged.
    """
    ...
    def record_ack(self, watch_id):
        self.acked += 1
        self._unique.add(watch_id)
...
class FenceSignal:
    """One-shot flag flipped by the SIGTERM handler in --stream mode.

    SIGTERM is the harness's FENCE: stop reading, flush in-flight, summarize.
    The handler does almost nothing (set a bool); the read loop observes it and
    unwinds cleanly so flush()+summary still run — unlike the one-shot
    breadcrumb handler, which exits the process immediately.
    """
...
class StreamExhausted(Exception):
    """Dataset ran out BEFORE a fence — the no-wrap invariant (IC-7).

    A streaming run must be fenced by SIGTERM while data remains; reaching the
    end of the single pass means the dataset was undersized. Wrapping to row 0
    would re-send rows and inflate the distinct-WatchID count, so this is a loud
    error, never a silent restart.
    """


def drive_stream(rows, bucket, fence, *, produce, poll, sleep):
    """Single-pass, token-bucket-paced stream driver. Pure of Kafka: every side
    effect goes through an injected callable, so pacing / fence / no-wrap are
    unit-tested without a broker.

    * ``rows``       : iterable of row dicts in deterministic file/row order.
    * ``bucket``     : TokenBucket (paces to --rate-limit).
    * ``fence``      : FenceSignal (SIGTERM sets ``.requested``).
    * ``produce(row)``: hand one row to the producer (serialize + produce +
                        local-queue backpressure).
    * ``poll(t)``    : drain delivery callbacks (also called during a pause so
                       acks land while paced).
    * ``sleep(s)``   : wait ``s`` seconds.

    Returns ``"fenced"`` when the fence stops it; raises :class:`StreamExhausted`
    if the iterator ends first (never wraps).
    """
    count = 0
    for row in rows:
        if fence.requested:
        ...
        while not bucket.try_acquire(1):
            if fence.requested:
                return "fenced"
            poll(0)
            sleep(bucket.time_until(1))
        produce(row)
        count += 1
        poll(0)
    raise StreamExhausted(
        f"dataset exhausted after {count} rows with no fence (SIGTERM) — a "
        f"stream must be fenced while data remains; the source is undersized. "
        f"Refusing to wrap (a wrap would re-send rows and inflate uniqueness).")


def validate_stream_args(stream, rate_limit):
    """``--stream`` requires a positive ``--rate-limit`` (the token-bucket rate).

    A continuous producer with no pace would run at full speed and exhaust the
    dataset immediately, tripping the no-wrap error — so refuse it up front.
    """
    if stream and (rate_limit is None or rate_limit <= 0):
        die("--stream requires --rate-limit N (rows/s, N > 0): a continuous "
            "producer must be paced.")
...
def parse_args():
    ...
    p.add_argument("--stream", action="store_true",
                   default=os.environ.get("STREAM", "false").lower() == "true",
                   help="Continuous streaming mode (chaos, IC-7): single-pass, "
                        "rate-limited, long-lived — runs until SIGTERM (the "
                        "fence), then flushes and prints a summary including "
                        "unique_sent. Requires --rate-limit. Never wraps.")
    p.add_argument("--rate-limit", type=int,
                   default=int(os.environ.get("RATE_LIMIT", "0")),
                   help="Token-bucket rate in rows/s for --stream (0 = unset).")
    return p.parse_args()
...
def _iter_rows(shard_dataset, field_order, string_fields, batch_size):
    """Yield rows in deterministic file/row order, single pass, under the
    UNCHANGED per-batch memory bounds (readahead 1x1 + prompt del) that the
    one-shot loop uses — streaming must not relax the OOM fix."""
...
def _stream_produce(args, shard_dataset, field_order, string_fields, mapper,
                    avro_serializer, ser_ctx, producer,
                    shard_index, shard_count, shard_files):
    """Continuous streaming path (IC-7). Single-pass, token-bucket-paced,
    fenced by SIGTERM; on the fence it flushes all in-flight and prints the
    final summary with unique_sent. Always exits the process."""
    # SIGTERM becomes the FENCE, overriding the one-shot breadcrumb handler
    # (which exits immediately). Here it only sets a flag; the read loop unwinds
    # cleanly so flush()+summary still run.
    fence = FenceSignal()
    try:
        signal.signal(signal.SIGTERM, fence.request)
    except (ValueError, OSError):
        pass

    bucket = TokenBucket(args.rate_limit)
    ledger = WatchIDLedger()
    sent = {"ok": 0, "err": 0}
    first_err = [None]

    def produce_row(row):
        # WatchID is threaded to the delivery callback so the ledger records it
        # only on ACK — unique_sent is the distinct set that reached the topic.
        wid = row[WATCHID_FIELD]
        payload = avro_serializer(mapper(row), ser_ctx)

        def _on_delivery(err, _msg, _wid=wid):
            if err is not None:
                sent["err"] += 1
                ...
                sent["ok"] += 1
                ledger.record_ack(_wid)

        # Same local-queue backpressure contract as the one-shot loop: on a full
        # rdkafka queue, poll (drain acks) and RETRY THE SAME row — never advance
        # past it. Idempotence keeps retries safe; the memory bound is unchanged.
        while True:
            try:
                ...
                producer.poll(0.5)
        _live_progress["read"] += 1
        r = _live_progress["read"]
        if r - _last_progress["read"] >= 500000:
            _last_progress["read"] = r
            log(f"streamed {r} rows (rate_limit={args.rate_limit}/s) "
                f"peak_rss={rss_mb():.0f} MiB")

    log(f"stream mode: single-pass at {args.rate_limit} rows/s; SIGTERM is the "
        f"fence (stop reading -> flush -> summary). Never wraps.")
    t0 = time.time()
    rows = _iter_rows(shard_dataset, field_order, string_fields, args.batch_size)
    try:
        drive_stream(rows, bucket, fence,
                     produce=produce_row,
                     poll=producer.poll,
                     sleep=time.sleep)
    except StreamExhausted as e:
        # No-wrap invariant: flush what we have so the log/offsets are honest,
        # then fail LOUDLY. rows_expected is offsets-derived, so this partial
        # flush corrupts nothing — but the run is INVALID (undersized dataset).
        producer.flush(300)
        die(str(e))

    log(f"fence received after {_live_progress['read']} rows; flushing...")
    ...
    summary = {
        ...
        "rows_sent": rows_sent,
        "unique_sent": ledger.unique_count,
    ...
    log(f"OK: stream fenced; rows_sent={rows_sent} "
        f"unique_sent={ledger.unique_count} in {duration:.1f}s")
    if rows_sent == 0:
        die("stream produced 0 rows before the fence", code=3)
    sys.exit(0)
...
def main():
    ...
    args = parse_args()
    validate_stream_args(args.stream, args.rate_limit)
    ...
    # Continuous streaming mode (chaos, IC-7): single-pass, rate-limited, fenced
    # by SIGTERM. Diverges from the one-shot backlog-preload path here and exits
    # inside the helper — the one-shot loop below is left byte-identical.
    if args.stream:
        _stream_produce(args, shard_dataset, field_order, string_fields, mapper,
                        avro_serializer, ser_ctx, producer,
                        shard_index, shard_count, shard_files)
        return  # unreachable — _stream_produce always exits the process
