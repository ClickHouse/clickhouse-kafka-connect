...
The connector supports an optional internal buffer (`bufferCount > 0`) that coalesces records from multiple `poll()` calls into a single ClickHouse insert. This amortizes per-insert overhead for low-throughput topics and topics whose throughput is capped by broker fetch limits.

Two flush strategies coexist:
...
### Recovering from mid-stream schema changes (`retryOnSchemaMismatchAttempts`)

RowBinary inserts are position-based: the connector serializes each record in the column order of its **cached** `Table` description, and ClickHouse deserializes the bytes assuming that same order. If the destination table's schema changes while the connector is running (e.g. an `ALTER TABLE ... ADD COLUMN`), the cached order no longer matches the server's order and the insert fails with a ClickHouse server error — most commonly code `33` (`CANNOT_READ_ALL_DATA`) or code `131` (`TOO_LARGE_STRING_SIZE`, because a misaligned byte is interpreted as a string-length prefix).

`ClickHouseWriter.doInsertRawBinary` already handles the common case: on error code `33`/`131` it refreshes the cached table mapping (`urgentTableUpdate`) and retries the insert **once**. When the DDL change is already visible to the node the connector talks to, this single in-process retry succeeds transparently.

That single retry is not always enough. On a multi-replica/cluster deployment the DDL change may not have propagated to every replica yet, so the refreshed mapping can still disagree with the replica that serves the retry, and the second attempt fails too.

The `retryOnSchemaMismatchAttempts` setting (allowed range `1`-`5`, default `1` — the previous single-retry behavior) generalizes that mechanism: `doInsertRawBinary` runs a bounded refresh-and-retry `for` loop of up to `retryOnSchemaMismatchAttempts` retries, with no delay between attempts — each cycle refreshes the mapping (`urgentTableUpdate`) and re-sends the insert. The retry happens at the lowest level of the stack, so nothing above it is re-executed: the batch is not rebuilt, the state machine is not re-entered, and the serialized bytes are simply written again. A schema mismatch is detected exactly as before: a V2 `ServerException` with code `33`/`131`, or (V1, deprecated) an exception message containing those codes. Only these failures consume the budget — anything else, including a `RetriableException` classified elsewhere in the stack (e.g. an insert timeout), propagates immediately and interrupts the loop, leaving framework-level retry behavior unchanged. Once the budget is exhausted, the exception from the **last** attempt propagates and `Utils.handleException` classifies it as it always has: codes `33`/`131` are not framework-retriable, so the failure is terminal (task fails, or the batch is routed to the DLQ depending on `errors.tolerance`). The hard cap of `5` (enforced both by `ConfigDef` validation and at config construction) protects against configuration mistakes that could stall the task in a long retry loop.

**Why re-sending the same bytes is safe:** codes `33`/`131` are raised while ClickHouse is *parsing* the incoming block, before any rows are committed, so a failed attempt writes nothing and the retry cannot duplicate data. Note the retry re-sends the batch serialized in the **stale** column order; it succeeds when the refreshed server-side schema once again matches that order (e.g. the DDL is rolled back) or, more commonly, when `urgentTableUpdate` picks up the new order and the next serialization aligns — the refresh happens before each re-send precisely so the attempt is made against the newest known mapping.

#### Caveats

- **Retries block the insert path.** The loop runs on the task thread inside `put()`. There is no sleep, so the cost per extra attempt is one mapping refresh plus one insert round-trip; a large budget against a permanently broken schema delays failure by (budget × round-trip time).
- **Sizing the budget.** Pick `N` large enough to cover your worst-case DDL propagation lag across replicas, and small enough that a permanent mismatch (incompatible type change, dropped column, malformed record) fails fast enough for your alerting — after the budget the failure is terminal or goes to the DLQ per `errors.tolerance`. Because there is no back-off between attempts, the whole budget can be consumed in well under a second on a fast network; if your DDL propagation lag is measured in seconds, size `N` accordingly.
- **Interaction with `auto.evolve`.** With `auto.evolve=true` the connector itself issues the `ADD COLUMN` DDL and then waits for propagation (`auto.evolve.ddl.refresh.retries`). `retryOnSchemaMismatchAttempts` is complementary: it covers the window where an *external* actor changed the schema, or where propagation outlasts the in-process retries.
